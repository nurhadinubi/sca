...
class BomApprovalController extends Controller
{
	...
	{
		$approvals = DB::table('bom_approvals as bap')
			->leftJoin('users as us', 'bap.user_id', '=', 'us.id')
			->leftJoin('doctype as dc', 'bap.doctype_id', 'dc.id')
			->select('bap.*', 'us.name', 'us.email', 'dc.doctype', 'dc.description')
			->orderBy('bap.transaction_type')
			->orderBy('bap.doctype_id')
			->orderBy('bap.level')
			->paginate(15);
	...
	public function needApprove()
	{
		...
		$idDoc = DB::table('bom_approvals')->where([
			'status' => 'P',
			'user_id' => Auth::user()->id,
		])->pluck('formula_header_id')->toArray();

		$formulaSemiFinish = DB::table('formula_header as fh')
			->select('u.name as requester', 'fh.*', 'sh.product_code as product_code')
			->leftJoin('scaleup_header as sh', 'sh.id', '=', 'fh.scaleup_header_id')
			->join('users as u', 'u.id', '=', 'fh.user_id')
			->where('fh.status', 'P')
			->whereIn('fh.id', $idDoc)
			->orderBy('fh.created_at', 'desc')
			->get();

		return view('approval.list-formula-sf', compact('formulaSemiFinish'));
	}
	...
	public function getApproval(Request $request)
	{
		...
		$header = DB::table('formula_header')->where([
			'doc_number' => $doc_number,
		])->first();

		$approver = DB::table('bom_approvals')->where('formula_header_id', $header->id)->orderBy('level')->get();
