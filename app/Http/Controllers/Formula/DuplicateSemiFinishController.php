...
class DuplicateSemiFinishController extends Controller
{
	...
	function duplicate($id)
	{
		...
		$header =  DB::table('formula_header as fh')
			...
			->leftJoin('sub_category as sc', 'sc.id', 'fh.doctype_id')
			->select('fh.*', 'sh.product_code', 'sh.material_id', 'u.name', 'sc.description as category_description')
			->where('fh.id', $keycode->formula_header_id)
			// ->where('fh.status', 'P')
		...
		if (!$header) {
			...
			return redirect()->route('sf.index')->with(['message' => [
				"type" => "error",
				"text" => "Anda tidak mempunyai authorisasi untuk mengedit Formula Tersebut"
			]]);
		}
